	...
	render(){
		...
		if (resultdata.length) {
		 	const listItems = resultdata.map((v, i) =>
                <li className='backlistItem' onClick={() => this.handleClick(v.id)} key={i}>
                    <div className='black-content-box'>
                        <div className='black-content-box-title'>
                            <span className={['black-content-box-title-markicon-zd ', type !== 'top' && v.isHot && yuyu]}>{type === 'top' ? '置顶' : (v.isHot ? '最新': '')}</span>
