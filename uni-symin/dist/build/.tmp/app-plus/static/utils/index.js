import EmojiObj from './message/emojimap'
wx.EmojiObj = EmojiObj;
wx.USER = {};
let tempid = '' // 定时器
export default {
  attachInfo(bool) {
  ...
  distance(x, y, x1, y1) {
	...
	let degy  = degfun(y)
	let degy1 = degfun(y1)
	let s = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin((degx - degx1)/2), 2) + Math.cos(x) * Math.cos(x1)*Math.pow(Math.sin((degy - degy1)/2), 2)))
    s = s * earthR;
