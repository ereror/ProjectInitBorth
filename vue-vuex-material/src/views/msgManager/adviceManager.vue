        ...
        mdb-card(class='col-md-12' style='padding-bottom:30px;')
          mdb-col(md="12")
            .flex-r.float-r.searchBox(md="4")
                ...
                mdb-input(class="mr-sm-2" type="text" placeholder="用户昵称" aria-label="姓名")
                mdb-btn(class="blue-gradient" rounded size="sm" type="submit" class="mr-auto") 查询
          mdb-col(md="12")
            el-table.el-table(:data="adviceData" border stripe size='small', max-height="'580'" style='width:100% !important;')
              ...
              el-table-column(label='投诉建议内容' align='center')
                template(slot-scope='scope')
                  div.flex-c
                    ...
                    div.flex-l(style='margin-left:1.5em;' v-for='(item, pindex) in scope.row.images', :key="pindex")
                      img(:src='item.path', style='width:60px;height:auto;margin-right:5px;')
  ...
  export default {
    ...
    methods: {
      ...
      getAdviseList () {
        ...
        msgServices.getAdviseList({page, pagesize}).then(res => {
          if (res.status === 200 && res.data && res.data.code === 200) {
            this.query.total = res.data && res.data.data && res.data.data.total
            this.adviceData = res.data && res.data.data && res.data.data.data
      ...
      saveBz (obj) {
        ...
        msgServices.setAdvise(temp).then(res => {
          if (res.status === 200 && res.data && res.data.code === 200) {
...
.bzimg {width:80px;height:80px;margin-right:5px;}
</style>
