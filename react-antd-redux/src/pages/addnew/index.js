...
const formstr = ['被曝光的名称/微信昵称不能为空', '标题不能超过15个字数', 
'曝光标题不能为空', '被曝光人微信号不能为空', '图片数量至少有一个最多九张', '电话号码只能11位', '被曝光人微信号只能是数字、下划线、中横线或字母', '曝光人微信号只能是数字或字母'
]
...
class addnew extends PureComponent {
	...
	onFileChange (files, type, index) {
		...
		if (type === 'add') {
		  ...
		  let type = file.type
		  if (type === 'image/gif') {
		  	return Toast.fail('图片上传不支持gif图片，请重新选择上传', 3)
		  }
	//	  if (file.file.size / 1024 > 1025) {
	//	  	return Toast.fail('图片不能大于1M, 请要压缩后重新上传。', 3)
	//	  }
	...
	saveData() {
		...
		function valiadata() {
			...
			if (that.state.distitle && that.state.distitle.length > 15) return iserror = 1
			...
			let execfunc = /^[A-Za-z0-9_-]+$/g
			if (!that.state.diswx.match(execfunc)) return iserror = 6
			if (!that.state.disnameT.match(execfunc)) return iserror = 7
			return iserror
		}
	}
	render() {
				...
				<div className='cardbox addnew02'>
					...
					<div className='addnew-img'>
			        	<ImagePicker files={files} onChange={this.onFileChange}
                          onImageClick={(index, fs) => console.log(index, fs)} onAddImageClick = {this.onImageClick}
