...
class Detail extends PureComponent {
    ...
    componentDidMount () {
    	...
    	let { id, openId } = state
		var sharetyperec = /(?:type=)(\w*)/ig
		var resultc = location.href.match(sharetyperec) || []
    	if (id && (!resultc || resultc && !resultc.length)) {
    		let userInfo = LocalStorageService.get('userInfo')
    		if (!openId) openId = userInfo.openId
    		this.state.id = id
	        ...
	        if (code) this.props.getUserInfo(code)
    	}
    }
    ...
    sharefunc(obj) {
    	var link = "http://"+ window.location.host + "/pwsq.html?id=" + obj.id
    	// 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx6357f5dafdb1c45d&redirect_uri=http%3a%2f%2fwww.hyjjdyg.com%2f%23%2fdetail%3fid%3d' + obj.id + '&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect',
    	wx.ready(function(){
    		// wx.updateAppMessageShareData({
    		// 	title: obj.title || '用户所举报的详情',
    		// 	desc: obj.content,
    		// 	link: link,
    		// 	imgUrl: obj.imgurl
    		// })
    ...
    getDetailTip (id) {
    	...
    	gzhService.detailTip({id, openId}).then(res => {
    		if (res.code * 1 === 200) {
    		    ...
    		    let imgsrc = results.imgs && results.imgs[0] && results.imgs[0].id
    		    let imgurl = imgsrc && (fileServer + imgsrc) || gzh
    		    results.imgurl = imgurl
    		    results.padnumArr = []
    		    results.imgs.map((v, i) => {
					let fileurl = fileServer + v.id
			        var image = new Image()
			        image.onload = function() {
			            ...
			            // console.log(padnum)
			            // 最后一个触发更新数据操作
			            if (results.imgs.length && (i === results.imgs.length - 1)) {
				    		that.setState({
				    			result : results
				    		}, () => {
				    			Toast.hide()
	...
	render(){
		...
		if (result.imgs && result.imgs.length) {
			imgList = result.imgs.map((v, i) => {
				let fileurl = fileServer + v.id
	            let padnum = Math.floor(v.height / v.width * 100) + '%'
		        ...
		        return (<div className={[' detailshowimg_01 ', orientation !== 1 && ' detailshowimg_02']}><div className='detailshowimg' style={imgstyle} keys={i}></div></div>)
			}
			)
		} else {
            let imgstyle = {
		...
		return (
			<div className='detailB'>
				...
				<div className='flex-b'>
					...
					<div className='distitleli distitleligray'>
						举报时间：{result.createDate}
					</div>
				</div>
				...
				<div className='distitleli'>事件内容(该事件由举报人描述与平台无关)</div>
